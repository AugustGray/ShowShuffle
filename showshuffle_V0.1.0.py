...
while True:
    ...
    for i, name in enumerate(series_list, start=1):
        print(f"{i}. {name}")
    print(f"{len(series_list) + 1}. Pick a random show")

    try:
        user_choice = input("\nEnter the number of the series you want to choose, or pick 'R' for random (or '0' to exit): ").strip().upper()
        
        if user_choice == "0":
            print("Goodbye!")
            break
        
        if user_choice == "R" or user_choice == str(len(series_list) + 1):
